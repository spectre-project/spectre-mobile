...
final spectrePriceProvider = Provider.autoDispose((ref) {
  return ref.watch(coingeckoSpectrePriceProvider);
});
...
final formatedTotalFiatProvider = Provider.autoDispose((ref) {
  final balance = ref.watch(totalBalanceProvider);
  final price = ref.watch(spectrePriceProvider);
...
final spectreFormatterProvider = Provider((ref) {
  ...
  final formatter = CurrencyFormatter(
    ...
    maxDecimalDigits: TokenInfo.spectre.decimals,
    maxAmount: kMaxSpectre,
  );
...
final fiatFormatterProvider = Provider.autoDispose((ref) {
  final price = ref.watch(spectrePriceProvider);
  final currency = ref.watch(currencyProvider);
  final maxAmount = price.price * kMaxSpectre;
